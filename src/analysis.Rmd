...
analysis_end_date <- as_date("2017-02-28") # last previous full month

active_licenses_daily_count <- active_licenses %>%
  mutate(ds = milestonedate) %>%
  filter(ds >= analysis_start_date & ds <= analysis_end_date) %>%
  group_by(ds) %>% 
  summarise(
    y = log(n())
  ) %>% 
...
We can calculate how many days it takes to issue a license for an applicant by pulling in additional relational data. 

## Data import

Business licensing milestones are comprised of a “log” recording change of status for each business license or application by date/time and inspector.
...
active_licenses <- active_licenses %>% 
  ...
  select(-x.x, -x.y) # remove intermediate columns used for calculation
...
What is the Days to Issue distribution?
